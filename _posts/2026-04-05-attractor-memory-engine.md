...
**The storage layer** uses PostgreSQL with pgvector. Each memory stores its content, context frame, tags, timestamps, access count, and a 384-dimensional embedding from a sentence-transformers model. The embedding enables semantic search — retrieving by meaning rather than exact words.
...
Port 7747 is "loci" on a phone keypad — a nod to the method of loci, the memory palace technique, which was the project's original name before I found a better one.
